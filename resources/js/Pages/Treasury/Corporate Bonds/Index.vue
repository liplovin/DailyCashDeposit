...
const filteredCorporateBonds = computed(() => {
    let filtered = props.corporateBonds;
    
    if (searchQuery.value.trim()) {
        const query = searchQuery.value.toLowerCase();
        filtered = filtered.filter(item => 
            item.corporate_bond_name.toLowerCase().includes(query) ||
...
const renewCorporateBond = async (bond) => {
    const result = await Swal.fire({
        ...
        html: `
            <div class="text-left">
                ...
                <p class="text-green-600 text-sm"><strong>✓ This will renew the corporate bond.</strong></p>
            </div>
        `,
        icon: 'question',
    ...
    if (result.isConfirmed) {
        router.post(`/treasury/corporate-bonds/${bond.id}/renew`, {}, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Renewed!',
...
const withdrawCorporateBond = async (bond) => {
    const result = await Swal.fire({
        title: 'Withdraw Corporate Bond?',
        html: `
            <div class="text-left">
                <p class="mb-3"><strong>Corporate Bond:</strong> ${bond.corporate_bond_name}</p>
                <p class="mb-3"><strong>Account:</strong> ${bond.account_number}</p>
                <p class="text-orange-600 text-sm"><strong>✓ This will withdraw the corporate bond amount.</strong></p>
            </div>
        `,
        icon: 'question',
        showCancelButton: true,
    ...
    if (result.isConfirmed) {
        router.post(`/treasury/corporate-bonds/${bond.id}/withdraw`, {}, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Withdrawn!',
                    text: 'Corporate Bond has been withdrawn successfully.',
                    icon: 'success',
                    confirmButtonColor: '#F59E0B',
                    timer: 2000,
                    timerProgressBar: true
            ...
            onError: () => {
                Swal.fire({
                    title: 'Error!',
                    text: 'Failed to withdraw corporate bond. Please try again.',
                    icon: 'error',
                    confirmButtonColor: '#F59E0B'
                });
            }
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-sm border-r border-gray-200">
                                    <div v-if="isMaturityActionVisible(bond.maturity_date)" class="flex items-center space-x-2">
                                        <button
                                            @click="renewCorporateBond(bond)"
                                        ...
                                        <button
                                            @click="withdrawCorporateBond(bond)"
