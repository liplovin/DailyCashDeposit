...
const remainingBalance = computed(() => {
    const currentBalance = parseFloat(props.corporateBond?.beginning_balance || 0);
...
const withdrawAll = () => {
    const balance = props.corporateBond?.beginning_balance || 0;
    form.value.amount = balance.toString();
};

// Watch for corporate bond changes and reset form
...
const handleSubmit = () => {
    ...
    router.post(
        `/treasury/corporate-bond/${props.corporateBond.id}/withdraw`,
        {
        ...
        },
        {
            onSuccess: () => {
                Swal.fire({
                    title: 'Withdrawn!',
                    text: 'Corporate bond withdrawal has been recorded successfully.',
                    icon: 'success',
        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Withdraw Corporate Bond</h2>
                    <button
                        @click="closeModal"
                ...
                <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
                    <!-- Corporate Bond Info -->
                    <div v-if="corporateBond" class="bg-orange-50 rounded-lg p-3 border border-orange-200">
                        ...
                        <p class="text-sm font-semibold text-gray-800 mt-1">
                            ...
                            ₱ {{ parseFloat(corporateBond.beginning_balance || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                        </p>
                        <p class="text-orange-600 text-sm font-semibold mt-2">
                            ⚠️ This action will withdraw the specified amount from the corporate bond.
                        </p>
                    </div>
                        ...
                        <div v-if="form.amount" class="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-bold text-gray-900">₱ {{ formatCurrency(corporateBond?.beginning_balance || 0) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 font-semibold">Remaining After Withdrawal</p>
                                    <p class="text-sm font-bold" :class="remainingBalance < 0 ? 'text-red-600' : 'text-blue-600'">
