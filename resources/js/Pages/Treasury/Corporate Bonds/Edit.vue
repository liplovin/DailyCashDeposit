...
// Get unique corporate bond names
const uniqueCorporateBonds = computed(() => {
    const seen = new Set();
    return props.existingCorporateBonds.filter(bond => {
        if (seen.has(bond.corporate_bond_name)) {
            return false;
        }
        seen.add(bond.corporate_bond_name);
        return true;
    });
});

// Watch for changes to the corporateBond prop and populate the form
watch(() => props.corporateBond, (newCorporateBond) => {
    if (newCorporateBond && newCorporateBond.id) {
        const existsInList = uniqueCorporateBonds.value.some(a => a.corporate_bond_name === newCorporateBond.corporate_bond_name);
        
        form.value = {
            corporate_bond_name: existsInList ? newCorporateBond.corporate_bond_name : 'other',
            account_number: newCorporateBond.account_number || '',
            beginning_balance: formatBalanceDisplay(newCorporateBond.beginning_balance),
            maturity_date: formatDateDisplay(newCorporateBond.maturity_date),
            acquisition_date: formatDateDisplay(newCorporateBond.acquisition_date),
            customCorporateBond: !existsInList ? newCorporateBond.corporate_bond_name : '',
            explanation: newCorporateBond.explanation || ''
        };
    }
}, { deep: true });
...
const handleSubmit = () => {
    ...
    // Determine the corporate bond name value to use
    let corporateBondValue = form.value.corporate_bond_name;
    if (form.value.corporate_bond_name === 'other') {
        corporateBondValue = form.value.customCorporateBond;
    }
    
    // Validation
    if (!corporateBondValue.trim()) {
        errors.value.corporate_bond_name = 'Corporate bond name is required';
    }
    ...
    if (Object.keys(errors.value).length === 0) {
        ...
        const submitData = {
            corporate_bond_name: corporateBondValue,
        ...
        router.put(`/treasury/corporate-bonds/${props.corporateBond.id}`, submitData, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Updated!',
                    text: 'Corporate Bond has been updated successfully.',
                    icon: 'success',
...
const closeModal = () => {
    form.value = { corporate_bond_name: '', account_number: '', beginning_balance: '', maturity_date: '', acquisition_date: '', customCorporateBond: '', explanation: '' };
    errors.value = {};
    emit('close');
};
        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Edit Corporate Bond</h2>
                    <button
                        @click="closeModal"
                ...
                <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
                    <!-- Corporate Bond Name Field -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Corporate Bond Name <span class="text-red-500">*</span>
                        </label>
                        <select
                            v-model="form.corporate_bond_name"
                            ...
                            :class="{ 'border-red-500 focus:ring-red-500': errors.corporate_bond_name }"
                        >
                            <option value="">-- Select Corporate Bond --</option>
                            <option v-for="bond in uniqueCorporateBonds" :key="bond.id" :value="bond.corporate_bond_name">
                                {{ bond.corporate_bond_name }}
                            </option>
                            <option value="other">Other (Please specify)</option>
                        </select>
                        <p v-if="errors.corporate_bond_name" class="text-red-500 text-sm mt-1">{{ errors.corporate_bond_name }}</p>
                    </div>

                    <!-- Custom Corporate Bond Field (appears when "Other" is selected) -->
                    <div v-if="form.corporate_bond_name === 'other'">
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Custom Corporate Bond Name <span class="text-red-500">*</span>
                        </label>
                        <input
                            v-model="form.customCorporateBond"
                            type="text"
                            placeholder="Enter custom corporate bond name"
