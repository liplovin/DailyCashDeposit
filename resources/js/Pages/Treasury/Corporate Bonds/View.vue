<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="handleClose">
      <div class="bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-y-auto" @click.stop>
      <!-- Header -->
      <div class="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex justify-between items-center">
        <h2 class="text-white text-lg font-semibold">Corporate Bond History</h2>
        <button
          type="button"
          @click="handleClose"
      ...
      <div class="p-6">
        ...
        <div v-if="activeTab === 'renewals'">
          <div v-if="corporateBond && corporateBond.renewals && corporateBond.renewals.length > 0" class="space-y-4">
            ...
            <div class="relative">
              <div class="space-y-6">
                <div
                  v-for="(renewal, index) in corporateBond.renewals"
                ...
                >
                  ...
                  <div v-if="index < corporateBond.renewals.length - 1" class="absolute left-2 top-6 w-0.5 h-12 bg-blue-200"></div>
        ...
        <div v-if="activeTab === 'withdrawals'">
          <div v-if="corporateBond && corporateBond.withdrawals && corporateBond.withdrawals.length > 0" class="space-y-4">
            <div
              v-for="withdrawal in corporateBond.withdrawals"
            ...
            >
              ...
              <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-orange-200">
                <div class="bg-white rounded-lg p-4 border border-orange-100">
                  ...
                  <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(corporateBond.beginning_balance + withdrawal.amount) }}</p>
                </div>
                ...
                <div class="bg-green-50 rounded-lg p-4 border border-green-300">
                  ...
                  <p class="text-xl font-bold text-green-700">₱ {{ formatCurrency(corporateBond.beginning_balance) }}</p>
                </div>
              </div>
        ...
        <div v-if="activeTab === 'balances'">
          <div v-if="corporateBond && corporateBond.balances && corporateBond.balances.length > 0" class="space-y-4">
            <div
              v-for="balance in corporateBond.balances"
            ...
            >
              ...
              <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-green-200">
                <div class="bg-white rounded-lg p-4 border border-green-100">
                  <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Previous Balance</p>
                  <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(corporateBond.beginning_balance - balance.amount) }}</p>
                </div>
                ...
                <div class="bg-blue-50 rounded-lg p-4 border border-blue-300">
                  <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">New Balance</p>
                  <p class="text-xl font-bold text-blue-700">₱ {{ formatCurrency(corporateBond.beginning_balance) }}</p>
                </div>
              </div>
