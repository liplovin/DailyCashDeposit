...
const handleSubmit = () => {
    ...
    router.post(
        `/treasury/corporate-bond/${props.corporateBond.id}/add-balance`,
        {
        ...
        },
        {
            onSuccess: () => {
                Swal.fire({
                    title: 'Success!',
                    text: `Added ${formatCurrency(parseFloat(form.value.amount))} to beginning balance.`,
                    icon: 'success',
                ...
                <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
                    ...
                    <div v-if="corporateBond" class="bg-yellow-50 rounded-lg p-3 border border-yellow-200">
                        ...
                        <p class="text-sm font-semibold text-gray-800 mt-1">
                            ...
                            <span class="text-blue-600">{{ formatCurrency(corporateBond.beginning_balance) }}</span>
                        </p>
                    </div>
                    ...
                    <div v-if="form.amount && parseFloat(form.amount) > 0" class="bg-green-50 rounded-lg p-3 border border-green-200">
                        ...
                        <p class="text-lg font-semibold text-green-600">
                            {{ formatCurrency(parseFloat(corporateBond.beginning_balance || 0) + parseFloat(form.amount || 0)) }}
                        </p>
                    </div>
