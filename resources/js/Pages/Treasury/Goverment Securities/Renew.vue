...
watch(() => props.governmentSecurity, (newSecurity) => {
    if (newSecurity && props.isOpen) {
        form.value = {
            new_acquisition_date: formatDateForInput(newSecurity.maturity_date),
            new_maturity_date: '',
            explanation: ''
        };
        errors.value = {};
...
const handleSubmit = () => {
    ...
    if (form.value.new_maturity_date && props.governmentSecurity?.maturity_date && !errors.value.new_maturity_date) {
        const selectedDate = new Date(form.value.new_maturity_date);
