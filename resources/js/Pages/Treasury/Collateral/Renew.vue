...
watch(() => props.collateral, (newCollateral) => {
    if (newCollateral && props.isOpen) {
        form.value = {
            new_acquisition_date: formatDateForInput(newCollateral.maturity_date),
            new_maturity_date: '',
            explanation: ''
        };
        errors.value = {};
...
const handleSubmit = () => {
    ...
    if (form.value.new_maturity_date && props.collateral?.maturity_date && !errors.value.new_maturity_date) {
        const newDate = new Date(form.value.new_maturity_date);
