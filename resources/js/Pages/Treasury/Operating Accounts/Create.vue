...
// Get unique operating account names
const uniqueOperatingAccounts = computed(() => {
    const seen = new Set();
    return props.existingOperatingAccounts.filter(account => {
        if (seen.has(account.operating_account_name)) {
            return false;
        }
        seen.add(account.operating_account_name);
        return true;
    });
});
...
const handleSubmit = () => {
    ...
    // Determine the operating account name value to use
    let operatingAccountValue = form.value.operating_account_name;
    if (form.value.operating_account_name === 'other') {
        operatingAccountValue = form.value.customOperatingAccount;
    }
    
    // Validation
    if (!operatingAccountValue.trim()) {
        errors.value.operating_account_name = 'Operating account name is required';
    }
    ...
    if (Object.keys(errors.value).length === 0) {
        ...
        router.post('/treasury/operating-accounts', submitData, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Created!',
                    text: 'Operating Account has been created successfully.',
                    icon: 'success',
...
const closeModal = () => {
    form.value = { operating_account_name: '', account_number: '', beginning_balance: '', maturity_date: '', acquisition_date: '', explanation: '', customOperatingAccount: '' };
    errors.value = {};
    emit('close');
};
        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Create New Operating Account</h2>
                    <button
                        @click="closeModal"
                ...
                <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
                    <!-- Operating Account Name Field -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Operating Account Name <span class="text-red-500">*</span>
                        </label>
                        <select
                            v-model="form.operating_account_name"
                            ...
                            :class="{ 'border-red-500 focus:ring-red-500': errors.operating_account_name }"
                        >
                            <option value="" disabled>-- Select Operating Account --</option>
                            <option v-for="account in uniqueOperatingAccounts" :key="account.id" :value="account.operating_account_name">
                                {{ account.operating_account_name }}
                            </option>
                            <option value="other">Other (Please specify)</option>
                        </select>
                        <p v-if="errors.operating_account_name" class="text-red-500 text-sm mt-1">{{ errors.operating_account_name }}</p>
                    </div>

                    <!-- Custom Operating Account Input (shown when "Other" is selected) -->
                    <div v-if="form.operating_account_name === 'other'">
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Enter Operating Account Name <span class="text-red-500">*</span>
                        </label>
                        <input
                            v-model="form.customOperatingAccount"
                            type="text"
                            placeholder="Enter custom operating account name"
