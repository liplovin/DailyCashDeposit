        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-y-auto">
                ...
                <div class="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex items-center justify-between z-10">
                    <h2 class="text-white text-lg font-semibold">Cash Infusion History</h2>
                    <button
                        @click="$emit('close')"
                ...
                <div class="p-6">
                    ...
                    <div v-if="activeTab === 'details'" class="space-y-6">
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="bg-blue-50 rounded-lg p-6 border border-blue-200">
                                <p class="text-xs text-gray-600 font-bold uppercase tracking-widest mb-2">Infusion Name</p>
                                <p class="text-lg font-bold text-gray-900">{{ cashInfusion.cash_infusion_name || '—' }}</p>
                            </div>

                            <!-- Account Number -->
                            <div class="bg-blue-50 rounded-lg p-6 border border-blue-200">
                                <p class="text-xs text-gray-600 font-bold uppercase tracking-widest mb-2">Account Number</p>
                                <p class="text-lg font-bold text-gray-900">{{ cashInfusion.account_number || '—' }}</p>
                            </div>

                            <!-- Acquisition Date -->
                            <div class="bg-green-50 rounded-lg p-6 border border-green-200">
                                <p class="text-xs text-gray-600 font-bold uppercase tracking-widest mb-2">Acquisition Date</p>
                                <p class="text-lg font-bold text-green-700">{{ formatDate(cashInfusion.acquisition_date) }}</p>
                            </div>
                            ...
                            <div class="bg-blue-100 rounded-lg p-6 border border-blue-300">
                                ...
                                <p class="text-lg font-bold text-blue-700">{{ formatDate(cashInfusion.maturity_date) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-lg p-6 border border-gray-200">
                            ...
                            <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap break-words">{{ cashInfusion.explanation?.trim() || '—' }}</p>
                        </div>
                    </div>

                    <!-- Renewal History Tab -->
                    <div v-if="activeTab === 'renewals'">
                        <div v-if="cashInfusion?.renewals && cashInfusion.renewals.length > 0" class="space-y-4">
                            <!-- Timeline -->
                            <div class="relative">
                                <div class="space-y-6">
                                    <div
                                        v-for="(renewal, index) in cashInfusion.renewals"
                                    ...
                                    >
                                        ...
                                        <div v-if="index < cashInfusion.renewals.length - 1" class="absolute left-2 top-6 w-0.5 h-12 bg-blue-200"></div>
                    ...
                    <div v-if="activeTab === 'withdrawals'">
                        <div v-if="cashInfusion?.withdrawals && cashInfusion.withdrawals.length > 0" class="space-y-4">
                            <div
                                v-for="withdrawal in cashInfusion.withdrawals"
                            ...
                            >
                                ...
                                <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-orange-200">
                                    <div class="bg-white rounded-lg p-4 border border-orange-100">
                                        ...
                                        <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(cashInfusion.beginning_balance + withdrawal.amount) }}</p>
                                    </div>
                                    ...
                                    <div class="bg-green-50 rounded-lg p-4 border border-green-300">
                                        ...
                                        <p class="text-xl font-bold text-green-700">₱ {{ formatCurrency(cashInfusion.beginning_balance) }}</p>
                                    </div>
                                </div>
                    ...
                    <div v-if="activeTab === 'balances'">
                        <div v-if="cashInfusion?.balances && cashInfusion.balances.length > 0" class="space-y-4">
                            <div
                                v-for="balance in cashInfusion.balances"
                            ...
                            >
                                ...
                                <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-green-200">
                                    <div class="bg-white rounded-lg p-4 border border-green-100">
                                        <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Previous Balance</p>
                                        <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(cashInfusion.beginning_balance - balance.amount) }}</p>
                                    </div>
                                    ...
                                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-300">
                                        <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">New Balance</p>
                                        <p class="text-xl font-bold text-blue-700">₱ {{ formatCurrency(cashInfusion.beginning_balance) }}</p>
                                    </div>
                                </div>
