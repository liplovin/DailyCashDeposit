...
// Get unique cash infusion names
const uniqueCashInfusions = computed(() => {
    const seen = new Set();
    return props.existingCashInfusions.filter(infusion => {
        if (seen.has(infusion.cash_infusion_name)) {
            return false;
        }
        seen.add(infusion.cash_infusion_name);
        return true;
    });
});

// Watch for changes to the cashInfusion prop and populate the form
watch(() => props.cashInfusion, (newCashInfusion) => {
    if (newCashInfusion && newCashInfusion.id) {
        const existsInList = uniqueCashInfusions.value.some(a => a.cash_infusion_name === newCashInfusion.cash_infusion_name);
...
const handleSubmit = () => {
    ...
    // Determine the cash infusion name value to use
    let cashInfusionValue = form.value.cash_infusion_name;
    if (form.value.cash_infusion_name === 'other') {
        cashInfusionValue = form.value.customCashInfusion;
    }
    
    // Validation
    if (!cashInfusionValue.trim()) {
        errors.value.cash_infusion_name = 'Cash infusion name is required';
    }
    ...
    if (Object.keys(errors.value).length === 0) {
        ...
        router.put(`/treasury/cash-infusion/${props.cashInfusion.id}`, submitData, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Updated!',
                    text: 'Cash Infusion has been updated successfully.',
                    icon: 'success',
        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Edit Cash Infusion</h2>
                    <button
                        @click="closeModal"
                ...
                <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
                    <!-- Cash Infusion Name Field -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Cash Infusion Name <span class="text-red-500">*</span>
                        </label>
                        <select
                            v-model="form.cash_infusion_name"
                            ...
                            :class="{ 'border-red-500 focus:ring-red-500': errors.cash_infusion_name }"
                        >
                            <option value="">-- Select Cash Infusion --</option>
                            <option v-for="infusion in uniqueCashInfusions" :key="infusion.id" :value="infusion.cash_infusion_name">
                                {{ infusion.cash_infusion_name }}
                            </option>
                            <option value="other">Other (Please specify)</option>
                        </select>
                        <p v-if="errors.cash_infusion_name" class="text-red-500 text-sm mt-1">{{ errors.cash_infusion_name }}</p>
                    </div>

                    <!-- Custom Cash Infusion Field (appears when "Other" is selected) -->
                    <div v-if="form.cash_infusion_name === 'other'">
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Custom Cash Infusion Name <span class="text-red-500">*</span>
                        </label>
                        <input
                            v-model="form.customCashInfusion"
                            type="text"
                            placeholder="Enter custom cash infusion name"
