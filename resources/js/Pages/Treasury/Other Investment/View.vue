<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-y-auto">
      <!-- Header -->
      <div class="sticky top-0 bg-gradient-to-r from-yellow-600 to-yellow-700 px-6 py-4 flex justify-between items-center">
        <h2 class="text-white text-lg font-semibold">Other Investment History</h2>
        <button
          @click="$emit('close')"
      ...
      <div class="p-6">
        ...
        <div v-if="activeTab === 'details'">
          <div v-if="otherInvestment" class="space-y-4">
            <!-- Other Investment Basic Info Grid -->
            <div class="grid grid-cols-2 gap-4">
              <div class="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Other Investment Name</p>
                <p class="text-sm font-medium text-gray-900">{{ otherInvestment.other_investment_name }}</p>
              </div>
              <div class="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Account Number</p>
                <p class="text-sm font-medium text-gray-900">{{ otherInvestment.account_number }}</p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                ...
                <p class="text-sm font-medium text-gray-900">{{ formatDate(otherInvestment.acquisition_date) }}</p>
              </div>
              <div class="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Maturity Date</p>
                <p class="text-sm font-medium text-gray-900">{{ formatDate(otherInvestment.maturity_date) }}</p>
              </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-4 border border-yellow-100">
              ...
              <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap break-words">{{ otherInvestment.explanation?.trim() || '—' }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'renewals'">
          <div v-if="otherInvestment.renewals && otherInvestment.renewals.length > 0" class="space-y-4">
            <!-- Timeline -->
            <div class="relative">
              <div class="space-y-6">
                <div
                  v-for="(renewal, index) in otherInvestment.renewals"
                  :key="`renewal-${renewal.id}`"
                  class="relative flex items-start"
                >
                  <!-- Timeline dot and line -->
                  <div class="absolute left-0 top-2 w-4 h-4 bg-yellow-600 rounded-full border-4 border-white"></div>
                  <div v-if="index < otherInvestment.renewals.length - 1" class="absolute left-2 top-6 w-0.5 h-12 bg-yellow-200"></div>
                  ...
                  <div class="ml-8 flex-1 bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="grid grid-cols-2 gap-4 mb-3">
                      ...
                      <div>
                        ...
                        <p class="text-sm font-medium text-yellow-600">{{ formatDate(renewal.new_maturity_date) }}</p>
                      </div>
                    </div>
                    
                    <div class="bg-white rounded-lg p-4 border border-yellow-100 mb-3">
                      <p class="text-xs text-gray-600 font-bold uppercase tracking-widest mb-2">Explanation</p>
        ...
        <div v-if="activeTab === 'withdrawals'">
          <div v-if="otherInvestment.withdrawals && otherInvestment.withdrawals.length > 0" class="space-y-4">
            <div
              v-for="withdrawal in otherInvestment.withdrawals"
            ...
            >
              ...
              <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-orange-200">
                <div class="bg-white rounded-lg p-4 border border-orange-100">
                  ...
                  <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(otherInvestment.beginning_balance + withdrawal.amount) }}</p>
                </div>
                ...
                <div class="bg-green-50 rounded-lg p-4 border border-green-300">
                  ...
                  <p class="text-xl font-bold text-green-700">₱ {{ formatCurrency(otherInvestment.beginning_balance) }}</p>
                </div>
              </div>
        ...
        <div v-if="activeTab === 'balances'">
          <div v-if="otherInvestment.balances && otherInvestment.balances.length > 0" class="space-y-4">
            <div
              v-for="balance in otherInvestment.balances"
            ...
            >
              ...
              <div class="grid grid-cols-3 gap-3 mb-5 pb-5 border-b border-green-200">
                <div class="bg-white rounded-lg p-4 border border-green-100">
                  <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">Previous Balance</p>
                  <p class="text-xl font-bold text-gray-800">₱ {{ formatCurrency(otherInvestment.beginning_balance - balance.amount) }}</p>
                </div>
                ...
                <div class="bg-blue-50 rounded-lg p-4 border border-blue-300">
                  <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">New Balance</p>
                  <p class="text-xl font-bold text-blue-700">₱ {{ formatCurrency(otherInvestment.beginning_balance) }}</p>
                </div>
              </div>
