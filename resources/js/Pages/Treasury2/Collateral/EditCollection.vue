...
watch(() => props.collateral, (newCollateral) => {
    if (newCollateral && props.isOpen) {
        amount.value = newCollateral.collection || 0;
    }
}, { deep: true, immediate: true });

watch(() => props.isOpen, (isOpen) => {
    if (isOpen && props.collateral) {
        amount.value = props.collateral.collection || 0;
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        await axios.put(`/treasury2/collateral/${props.collateral?.id}/collection`, {
    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden border-2 border-gray-200">
            ...
            <div class="px-6 py-6 space-y-4">
                <!-- Collateral Name -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Collateral Name</label>
                    <input
                        type="text"
                ...
                <div>
                    ...
                    <input
                        type="text"
                        :value="collateral?.account_number"
                        disabled
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg bg-gray-50 text-gray-700 font-medium text-sm cursor-not-allowed"
                    />
                </div>
