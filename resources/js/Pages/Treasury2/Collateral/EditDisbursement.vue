...
watch(() => props.collateral, (newCollateral) => {
    if (newCollateral && props.isOpen) {
        amount.value = newCollateral.disbursement || 0;
    }
}, { deep: true, immediate: true });

watch(() => props.isOpen, (isOpen) => {
    if (isOpen && props.collateral) {
        amount.value = props.collateral.disbursement || 0;
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        await axios.put(`/treasury2/collateral/${props.collateral?.id}/disbursement`, {
