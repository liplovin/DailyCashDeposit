...
const rollingBeginningBalance = computed(() => {
    if (!props.timeDeposit || !props.filterDate) {
        return parseFloat(props.timeDeposit?.beginning_balance || 0);
    }
    
    let balance = parseFloat(props.timeDeposit.beginning_balance || 0);
    
    // Add collections before the selected date
    const collectionDate = props.timeDeposit.collection_date 
        ? new Date(props.timeDeposit.collection_date).toISOString().split('T')[0] 
        : null;
    if (collectionDate && collectionDate < props.filterDate) {
        balance += parseFloat(props.timeDeposit.collection || 0);
    }
    
    // Subtract disbursements before the selected date
    const disbursementDate = props.timeDeposit.disbursement_date 
        ? new Date(props.timeDeposit.disbursement_date).toISOString().split('T')[0] 
        : null;
    if (disbursementDate && disbursementDate < props.filterDate) {
        balance -= parseFloat(props.timeDeposit.disbursement || 0);
    }
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.post(`/treasury2/time-deposit/${props.timeDeposit?.id}/collection`, {
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });

        emit('submit', {
            timeDeposit_id: props.timeDeposit?.id,
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });
        amount.value = '';
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full animation-fade-in overflow-hidden">
            ...
            <div class="px-6 py-6 space-y-5">
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200 rounded-xl p-5">
                    ...
                    <p class="text-lg font-bold text-green-900">{{ timeDeposit?.account_name || 'N/A' }}</p>
                    <p class="text-sm text-green-700 mt-2 font-mono">{{ timeDeposit?.account_number }}</p>
