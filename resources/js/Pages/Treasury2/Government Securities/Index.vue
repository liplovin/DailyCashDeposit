...
const handleCollection = (security) => {
    Swal.fire({
        title: 'Collection',
        text: `Add collection for ${security.government_security_name}`,
        icon: 'info',
...
const handleDisbursement = (security) => {
    Swal.fire({
        title: 'Disbursement',
        text: `Add disbursement for ${security.government_security_name}`,
        icon: 'info',
...
const totalCollection = computed(() => {
    return filteredSecurities.value.reduce((sum, security) => {
        return sum + parseFloat(security.collection || 0);
    }, 0);
});

const totalDisbursement = computed(() => {
    return filteredSecurities.value.reduce((sum, security) => {
        return sum + parseFloat(security.disbursement || 0);
    }, 0);
});

const totalEndingBalance = computed(() => {
    return filteredSecurities.value.reduce((sum, security) => {
        const ending = parseFloat(security.beginning_balance || 0) + parseFloat(security.collection || 0) - parseFloat(security.disbursement || 0);
        return sum + ending;
    }, 0);
});
</script>
    ...
    <Treasury2Layout>
        <div class="w-full px-8 py-6">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-600 text-sm font-medium">Manage government securities portfolio with real-time balance and maturity date tracking</p>
            </div>
                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 font-semibold border-r border-gray-200">{{ formatCurrency(security.beginning_balance) }}</td>
                                <td class="px-6 py-4 text-sm text-green-600 font-semibold border-r border-gray-200">{{ formatCurrency(security.collection) }}</td>
                                <td class="px-6 py-4 text-sm text-red-600 font-semibold border-r border-gray-200">{{ formatCurrency(security.disbursement) }}</td>
                                <td class="px-6 py-4 text-sm text-blue-600 font-semibold border-r border-gray-200">{{ formatCurrency(parseFloat(security.beginning_balance || 0) + parseFloat(security.collection || 0) - parseFloat(security.disbursement || 0)) }}</td>
