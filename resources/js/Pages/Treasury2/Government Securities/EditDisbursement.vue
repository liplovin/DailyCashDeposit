...
watch(() => props.governmentSecurity, (newSecurity) => {
    if (newSecurity && props.isOpen) {
        amount.value = newSecurity.disbursement || 0;
    }
}, { deep: true, immediate: true });

watch(() => props.isOpen, (isOpen) => {
    if (isOpen && props.governmentSecurity) {
        amount.value = props.governmentSecurity.disbursement || 0;
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        await axios.put(`/treasury2/government-securities/${props.governmentSecurity?.id}/disbursement`, {
