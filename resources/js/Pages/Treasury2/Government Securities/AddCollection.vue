...
const rollingBeginningBalance = computed(() => {
    if (!props.governmentSecurity || !props.filterDate) {
        return parseFloat(props.governmentSecurity?.beginning_balance || 0);
    }
    
    let balance = parseFloat(props.governmentSecurity.beginning_balance || 0);
    
    // Add collections before the selected date
    const collectionDate = props.governmentSecurity.collection_date 
        ? new Date(props.governmentSecurity.collection_date).toISOString().split('T')[0] 
        : null;
    if (collectionDate && collectionDate < props.filterDate) {
        balance += parseFloat(props.governmentSecurity.collection || 0);
    }
    
    // Subtract disbursements before the selected date
    const disbursementDate = props.governmentSecurity.disbursement_date 
        ? new Date(props.governmentSecurity.disbursement_date).toISOString().split('T')[0] 
        : null;
    if (disbursementDate && disbursementDate < props.filterDate) {
        balance -= parseFloat(props.governmentSecurity.disbursement || 0);
    }
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.post(`/treasury2/government-security/${props.governmentSecurity?.id}/collection`, {
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });

        emit('submit', {
            governmentSecurity_id: props.governmentSecurity?.id,
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });
        amount.value = '';
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full animation-fade-in overflow-hidden">
            ...
            <div class="px-6 py-6 space-y-5">
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200 rounded-xl p-5">
                    ...
                    <p class="text-lg font-bold text-green-900">{{ governmentSecurity?.government_security_name || 'N/A' }}</p>
                    <p class="text-sm text-green-700 mt-2 font-mono">{{ governmentSecurity?.reference_number }}</p>
