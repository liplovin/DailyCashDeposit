...
const rollingBeginningBalance = computed(() => {
    if (!props.cashInfusion || !props.filterDate) {
        return parseFloat(props.cashInfusion?.beginning_balance || 0);
    }
    
    let balance = parseFloat(props.cashInfusion.beginning_balance || 0);
    
    // Add collections before the selected date
    const collectionDate = props.cashInfusion.collection_date 
        ? new Date(props.cashInfusion.collection_date).toISOString().split('T')[0] 
        : null;
    if (collectionDate && collectionDate < props.filterDate) {
        balance += parseFloat(props.cashInfusion.collection || 0);
    }
    
    // Subtract disbursements before the selected date
    const disbursementDate = props.cashInfusion.disbursement_date 
        ? new Date(props.cashInfusion.disbursement_date).toISOString().split('T')[0] 
        : null;
    if (disbursementDate && disbursementDate < props.filterDate) {
        balance -= parseFloat(props.cashInfusion.disbursement || 0);
    }
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.post(`/treasury2/cash-infusion/${props.cashInfusion?.id}/collection`, {
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });

        emit('submit', {
            cashInfusion_id: props.cashInfusion?.id,
            amount: parseFloat(amount.value),
            date: props.filterDate || new Date().toISOString().split('T')[0]
        });
        amount.value = '';
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full animation-fade-in overflow-hidden">
            ...
            <div class="px-6 py-6 space-y-5">
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200 rounded-xl p-5">
                    ...
                    <p class="text-lg font-bold text-green-900">{{ cashInfusion?.cash_infusion_name || 'N/A' }}</p>
                    <p class="text-sm text-green-700 mt-2 font-mono">{{ cashInfusion?.account_number }}</p>
