...
watch(() => props.corporateBond, (newBond) => {
    if (newBond && props.isOpen) {
        amount.value = newBond.collection || 0;
    }
}, { deep: true, immediate: true });

watch(() => props.isOpen, (isOpen) => {
    if (isOpen && props.corporateBond) {
        amount.value = props.corporateBond.collection || 0;
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        await axios.put(`/treasury2/corporate-bonds/${props.corporateBond?.id}/collection`, {
    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div class="px-6 py-6 space-y-4">
                <!-- Corporate Bond Name -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Corporate Bond Name</label>
                    <input
                        type="text"
                        :value="corporateBond?.corporate_bond_name"
                        disabled
                ...
                <div>
                    ...
                    <input
                        type="text"
                        :value="corporateBond?.account_number"
                        disabled
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg bg-gray-50 text-gray-700 font-medium text-sm cursor-not-allowed"
                    />
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        step="0.01"
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg bg-white text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"
                    />
                </div>
            </div>
            ...
            <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t-2 border-gray-200">
                ...
                <button
                    ...
                    class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg font-semibold hover:shadow-lg hover:shadow-blue-500/40 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-200"
                >
