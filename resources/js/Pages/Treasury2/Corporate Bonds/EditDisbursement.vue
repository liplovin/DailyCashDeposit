...
watch(() => props.corporateBond, (newBond) => {
    if (newBond && props.isOpen) {
        amount.value = newBond.disbursement || 0;
    }
}, { deep: true, immediate: true });

watch(() => props.isOpen, (isOpen) => {
    if (isOpen && props.corporateBond) {
        amount.value = props.corporateBond.disbursement || 0;
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        await axios.put(`/treasury2/corporate-bonds/${props.corporateBond?.id}/disbursement`, {
