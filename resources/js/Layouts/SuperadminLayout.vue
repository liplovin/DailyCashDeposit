...
import { Home, LogOut, Menu, Inbox, CreditCard, Users } from 'lucide-vue-next';
...
onMounted(() => {
    const savedState = localStorage.getItem('superadmin_sidebar_open');
    if (savedState !== null) {
...
watch(sidebarOpen, (newValue) => {
    localStorage.setItem('superadmin_sidebar_open', JSON.stringify(newValue));
});
...
const isRouteActive = (routeName) => {
    const routeList = {
        'dashboard': '/dashboard',
        'superadmin.ops-collection': '/superadmin/ops-collection',
        'superadmin.ops-disbursement': '/superadmin/ops-disbursement',
...
const menuItems = [
    ...
    },
    {
        name: 'Ops Collection',
        component: Inbox,
        route: 'superadmin.ops-collection'
    },
    {
        name: 'Ops Disbursement',
        component: CreditCard,
        route: 'superadmin.ops-disbursement'
    },
    {
            ...
            <nav class="flex-1 px-2 py-8 space-y-2 overflow-y-auto">
                ...
                <div v-for="(item, index) in menuItems" :key="index">
                    ...
                    >
                        ...
                        <div v-if="isRouteActive(item.route) && sidebarOpen" class="w-1 h-1 rounded-full flex-shrink-0 bg-yellow-600"></div>
                    </button>
                </div>
            </nav>
                ...
                <div v-if="sidebarOpen" class="bg-gradient-to-br from-yellow-50 to-amber-50 border border-yellow-100 rounded-xl p-4 transition-opacity duration-300">
                    <div class="flex items-center space-x-3 mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-600 font-medium">Super Administrator</p>
                        </div>
                    </div>
                </div>
